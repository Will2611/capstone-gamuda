...
export function OwnerDashboard() {
  const metricsData = [
    { label: 'Visibility Score', value: 87, icon: <TrendingUp size={24} />, color: 'bg-bs-green' },
    { label: 'Match Conversion', value: 65, icon: <Search size={24} />, color: 'bg-bs-gold' },
    { label: 'Avg. Rating', value: 4.7, icon: <Tag size={24} />, color: 'bg-bs-blue' },
  ];
  ...
  const heatmapData = [
    { hour: '11 AM', Mon: 20, Tue: 25, Wed: 30, Thu: 28, Fri: 45, Sat: 60, Sun: 50 },
    { hour: '12 PM', Mon: 45, Tue: 48, Wed: 52, Thu: 50, Fri: 65, Sat: 80, Sun: 75 },
    { hour: '1 PM', Mon: 50, Tue: 52, Wed: 55, Thu: 53, Fri: 70, Sat: 85, Sun: 78 },
    { hour: '6 PM', Mon: 60, Tue: 65, Wed: 68, Thu: 85, Fri: 95, Sat: 100, Sun: 80 },
    { hour: '7 PM', Mon: 75, Tue: 78, Wed: 80, Thu: 90, Fri: 100, Sat: 95, Sun: 70 },
  ];
          ...
          {metricsData.map((metric, index) => (
            <Card key={index}>
              <div className="flex items-start justify-between">
                <div>
                  <p className="text-sm text-bs-neutral-600 mb-1">{metric.label}</p>
                  <h2>{typeof metric.value === 'number' && metric.value < 10 ? metric.value.toFixed(1) : metric.value}{metric.label === 'Match Conversion' ? '%' : metric.label === 'Visibility Score' ? '/100' : ''}</h2>
                </div>
          ...
          <Card>
            ...
            <ResponsiveContainer width="100%" height={250}>
              <LineChart data={trendData}>
                ...
                <YAxis stroke="#737373" />
                <Tooltip />
                <Line type="monotone" dataKey="searches" stroke="#2D9CDB" strokeWidth={2} />
                <Line type="monotone" dataKey="conversions" stroke="#27AE60" strokeWidth={2} />
              </LineChart>
            </ResponsiveContainer>
          </Card>
        </div>
        ...
        <Card className="mb-8">
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead>
                <tr>
                  <th className="text-left p-2 text-sm text-bs-neutral-600">Hour</th>
                  {['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'].map((day) => (
                    <th key={day} className="text-center p-2 text-sm text-bs-neutral-600">{day}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-bs-neutral-600">
                  Update your menu with "spicy noodles" to match trending searches
                </p>
              </div>
            </div>
          </Card>
