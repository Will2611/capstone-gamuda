...
import { useNotification } from "../PWABadge";
...
export function Navigation() {
  ...
  return (
    <>
      <nav className="bg-white border-b border-bs-neutral-200 sticky relative top-0 z-20">
        <div className="max-w-7xl mx-auto px-6 py-4">
          ...
          {menuOpen && (
            <ul className="md:hidden mt-4 pb-4 border-t border-bs-neutral-200 pt-4 sticky top-0">
              {navItems.map((item) => (
                <li key={item.path}>
                  <NavLink
                    to={item.path}
      ...
      {menuOpen && (
        <div
          className="absolute bottom-0 left-0 min-h-[calc(100vh-377px)] min-w-screen bg-gray-500 opacity-20 z-15"
          onClick={() => setMenuOpen(!menuOpen)}
        ></div>
      )}
    </>
